...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    ...
    async function loadPatientRecords() {
      try {
        const res = await medicalRecordApi.doctorPatient(selectedPatient._id, {
          page: recordsPage,
          limit: 5,
        });
  ...
  const loadData = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      const apptList =
        apptRes?.data?.appointments || (Array.isArray(apptRes) ? apptRes : []);
  ...
  const todays = useMemo(() => {
    ...
    return appointments
      .filter(
        (a) => a && a.date && getLocalISODate(new Date(a.date)) === todayStr,
      )
  ...
  const revenueData = useMemo(() => {
    ...
    completedList.forEach((appt) => {
      ...
      const key = dateObj.toLocaleString("en-US", {
        month: "short",
        year: "numeric",
      });
  ...
  const statusData = useMemo(() => {
    ...
    return [
      {
        name: "Pending",
        value: counts.pending + counts.booked,
        color: "#F59E0B",
      },
      ...
      {
        name: "Completed",
        value: counts.completed + counts.prescription_shared,
        color: "#0F6CBD",
      },
  ...
  const saveBlockedDates = async () => {
    ...
    try {
      ...
      setToast(
        e?.response?.data?.message || e?.message || "Failed to update leaves",
      );
      setToastType("error");
    } finally {
          ...
          >
            {sortedDates.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table
                  className="w-full text-left"
                  aria-label="List of leave dates"
                >
                  <thead>
                    <tr className="border-b border-slate-50">
                      <th
                    ...
                    {sortedDates.map((dateStr) => {
                      ...
                      return (
                        <tr
                          ...
                          className="group hover:bg-slate-50/50 transition-colors"
                        >
                          <td className="py-4 font-black text-secondary">
                            {formattedDate}
                          </td>
                          <td className="py-4 font-bold text-medical-text-light">
                            {dayName}
                          </td>
                          <td className="py-4 text-right">
                            <button
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-8">
        <DashboardWidget
          ...
          action={
            <button
              ...
              className="btn-primary !py-2 !text-[10px] focus-visible:ring-offset-2"
            >
              {savingAvailability ? "Syncing..." : "Publish Slots"}
            </button>
          }
        >
          <div
            className="space-y-4"
            role="list"
            aria-label="Available slot windows"
          >
            {availabilityRows.map((row, i) => (
              <div
                key={i}
                role="listitem"
                className="p-5 rounded-3xl border border-slate-100 bg-white shadow-sm grid grid-cols-2 md:grid-cols-5 gap-4 relative group focus-within:ring-2 focus-within:ring-primary outline-none"
              >
                <div className="space-y-1">
                  <label
                    htmlFor={`day-${i}`}
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const next = [...availabilityRows];
                      next[i].day = e.target.value;
                      setAvailabilityRows(next);
                    }}
                    className="medical-input !py-2 !text-xs focus:ring-primary"
                  >
                    {DAY_OPTIONS.map((d) => (
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      availabilityDirty.current = true;
                      const next = [...availabilityRows];
                      next[i].clinicName = e.target.value;
                      setAvailabilityRows(next);
                    }}
                    className="medical-input !py-2 !text-xs focus:ring-primary"
                  />
                ...
                <button
                  onClick={() => {
                    availabilityDirty.current = true;
                    setAvailabilityRows(
                      availabilityRows.filter((_, idx) => idx !== i),
                    );
                  }}
                  className="absolute -top-2 -right-2 w-6 h-6 bg-rose-50 text-rose-500 rounded-full flex items-center justify-center md:opacity-0 group-hover:opacity-100 transition-opacity border border-rose-100 shadow-sm focus:outline-none"
                  aria-label="Remove slot window"
                >
                  <svg
                    className="w-3 h-3"
                    fill="none"
            ...
            <button
              ...
              className="w-full py-4 rounded-3xl border-2 border-dashed border-slate-100 text-[10px] font-black text-primary uppercase tracking-widest hover:bg-slate-50 transition-all focus:ring-2 focus:ring-primary outline-none"
            >
              + Add Consultation Window
            </button>
          </div>
        </DashboardWidget>
        ...
        <div className="space-y-6">
          <DashboardWidget
            ...
            subtitle="Your credentials and fees"
          >
            <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  id="prof-bio"
                  rows={3}
                  placeholder="Write a short summary about your medical practice..."
                  value={profileForm.bio}
                  onChange={(e) => {
                    profileDirty.current = true;
                    setProfileForm({ ...profileForm, bio: e.target.value });
                  }}
                  className="medical-input !py-2.5 !text-xs focus:ring-primary resize-none"
                />
              </div>
              <button
                onClick={async () => {
                ...
                className="btn-primary w-full !py-3 !text-[10px] uppercase focus-visible:ring-offset-2"
              >
                {savingProfile ? "Syncing..." : "Update Professional Data"}
              </button>
            </form>
          </DashboardWidget>
        </div>
  ...
  const renderAnalytics = () => {
    if (!Array.isArray(appointments) || appointments.length === 0) {
      return (
        <div
          className="space-y-8 animate-in zoom-in-95 duration-700"
          role="tabpanel"
        ...
        >
          <div className="medical-card p-12 text-center flex flex-col items-center justify-center min-h-[300px]">
            ...
            <h3 className="text-lg font-black text-secondary tracking-tight mb-2">
              No Analytics Data Available
            </h3>
            <p className="text-xs font-semibold text-medical-text-light max-w-sm uppercase tracking-wide">
              Once you start receiving clinical bookings and completing
              consultations, your revenue and volume statistics will appear
              here.
            </p>
          </div>
        </div>
      ...
      >
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <DashboardWidget
            ...
            subtitle="Clinical revenue over time from completed visits"
            icon={
              <svg
                className="w-5 h-5"
                fill="none"
            ...
            {revenueData.length === 0 ? (
              <div className="h-64 flex flex-col items-center justify-center text-center p-6 border border-dashed border-slate-100 rounded-2xl bg-slate-50/50">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">
                  Complete appointments to start tracking revenue history.
                </p>
              </div>
            ) : (
            ...
            <div className="h-64 w-full flex flex-col sm:flex-row items-center justify-center gap-6">
              <div className="h-44 w-44 relative shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <span className="text-[9px] font-black text-medical-text-light uppercase tracking-widest">
                    Total
                  </span>
                </div>
              </div>
              ...
              >
                {statusData.map((entry, index) => (
                  ...
                  >
                    ...
                    <span className="font-black text-medical-text-light">
                      {entry.value} (
                      {Math.round((entry.value / appointments.length) * 100)}%)
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="flex items-center justify-between pb-2 border-b border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-medical-text-light font-medium">
              ...
              <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[9px] font-black bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded uppercase tracking-widest">
                      Verified History
                    </span>
                  </div>
                </div>
                {selectedPatient.patientProfile && (
                  <div className="space-y-3 pt-4 border-t border-slate-200">
                    ...
                    {selectedPatient.patientProfile.allergies &&
                      selectedPatient.patientProfile.allergies.length > 0 && (
                        <div>
                          <span className="text-[10px] font-black text-medical-text-light uppercase tracking-widest block mb-1">
                            Allergies
                          </span>
                          <div className="flex flex-wrap gap-1">
                            {selectedPatient.patientProfile.allergies.map(
                              (allergy, i) => (
                                <span
                                  key={i}
                                  className="px-2 py-0.5 bg-amber-50 border border-amber-100 text-amber-700 rounded text-[9px] font-black uppercase tracking-widest"
                                >
                                  {allergy}
                                </span>
                              ),
                    ...
                    {selectedPatient.patientProfile.chronicConditions &&
                      selectedPatient.patientProfile.chronicConditions.length >
                        0 && (
                        <div>
                          <span className="text-[10px] font-black text-medical-text-light uppercase tracking-widest block mb-1">
                            Chronic Conditions
                          ...
                          <div className="flex flex-wrap gap-1">
                            {selectedPatient.patientProfile.chronicConditions.map(
                              (cond, i) => (
                                <span
                                  key={i}
                                  className="px-2 py-0.5 bg-rose-50 border border-rose-100 text-rose-700 rounded text-[9px] font-black uppercase tracking-widest"
                                >
                                  {cond}
                                </span>
                              ),
              ...
              >
                <div className="space-y-4" role="list">
                  {patientRecords.length === 0 ? (
                    <div
                      className="py-20 text-center text-xs font-bold text-slate-400 uppercase tracking-widest"
                      role="status"
                    >
                    ...
                    patientRecords.map((rec) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <h4 className="font-black text-secondary group-hover:text-primary transition-colors">
                            {rec.diagnosis}
                          </h4>
                          <span className="text-[9px] font-black bg-emerald-50 text-emerald-600 px-2 py-1 rounded uppercase tracking-widest">
                            Record
                          </span>
                        </div>
                        ...
                        {rec.medicines && rec.medicines.length > 0 && (
                          <div className="mt-3 flex flex-wrap gap-2">
                            {rec.medicines.slice(0, 3).map((med, idx) => (
                              <div
                                key={idx}
                                className="px-2 py-1 bg-slate-50 rounded text-[10px] font-bold text-slate-500 uppercase tracking-tighter"
                              >
                                {med.name}
                              </div>
                            ))}
      ...
      <div className="space-y-6 animate-in slide-in-from-right-10 duration-700">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2">
          <div>
            ...
            <p className="text-xs text-medical-text-light font-medium">
              Verify your clinical lookup catalog and medical histories
            </p>
          </div>
        ...
        {filteredPatients.length === 0 ? (
          <div className="py-20 text-center border border-slate-100 rounded-3xl bg-slate-50/50">
            <p className="text-medical-text-light font-bold text-xs uppercase tracking-widest">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="truncate">
                    <h4 className="font-extrabold text-secondary text-sm group-hover:text-primary transition-colors truncate">
                      {pat.name}
                    </h4>
    ...
    >
      ...
      <div className="space-y-10">
        <div
          ...
          role="region"
          aria-label="Clinical metrics summary"
        >
          <DashboardStatCard
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-semibold text-medical-text-light uppercase tracking-wide">
                      Complete your profile checklist to activate public patient
                      booking.
                    </p>
                  </div>
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center justify-between">
                          <span
                            className={`text-[9px] font-black uppercase tracking-wider ${step.completed ? "text-emerald-600" : "text-slate-400"}`}
                          >
                            Step
                          </span>
                          {step.completed ? (
                            <svg
                              className="w-4 h-4 text-emerald-600 fill-emerald-100"
                              viewBox="0 0 20 20"
                              fill="currentColor"
                            >
                              <path
                        ...
                        <h4 className="font-extrabold text-xs tracking-tight mt-1">
                          {step.label}
                        </h4>
                          ...
                          onClick={() => {
                            ...
                            setTimeout(() => {
                              if (step.id === "availability") {
                                ...
                                document
                                  .getElementById("prof-qualifications")
                                  ?.scrollIntoView({ behavior: "smooth" });
                              }
                            }, 100);
                          }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-semibold text-emerald-600 uppercase tracking-wide mt-0.5">
                      Your clinical profile is fully setup. Public patient
                      booking is active!
                    </p>
                  </div>
                </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between">
                <div>
                  ...
                  <span className="font-extrabold text-secondary text-sm">
                    {prescribeTarget.patientId?.name || "Verified Patient"}
                  </span>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-3">
                  {rxMedicines.map((med, index) => (
                    <div
                      key={index}
                      className="flex gap-2 items-center bg-slate-50/50 p-3 rounded-2xl border border-slate-100/50 relative group"
                    >
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-start gap-4 pb-4 border-b border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-medical-text-light uppercase tracking-widest">
                    {selectedPatient?.email}
                  </p>
              ...
              {/* Medicines table */}
              <div>
                <span className="text-[9px] font-black text-medical-text-light uppercase tracking-widest block mb-2">
                        ...
                        selectedRecord.medicines.length > 0 ? (
                          selectedRecord.medicines.map((med, index) => (
                            <tr key={index} className="hover:bg-slate-50/50">
                              <td className="p-3 font-extrabold text-secondary">
                                {med.name}
                              </td>
                ...
                <div className="p-4 rounded-2xl bg-blue-50/30 border border-blue-100/50 space-y-2">
                  ...
                  <div className="space-y-2">
                    {selectedRecord.pdfs.map((pdf, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-lg bg-red-50 text-red-500 flex items-center justify-center font-black text-[10px]">
                            PDF
                          </div>
