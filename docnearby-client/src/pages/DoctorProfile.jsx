  ...
  const renderStars = (rating, size = "sm") => {
    ...
    return (
      <div className="flex gap-0.5" role="img" aria-label={`${rating} out of 5 stars`}>
        {[1, 2, 3, 4, 5].map((s) => (
          <svg key={s} className={`${starClass} ${s <= rating ? "fill-primary text-primary" : "text-slate-200"}`} viewBox="0 0 20 20" aria-hidden="true">
      ...
      <div className="section-container">
        {loading && !doctor && (
          <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4" aria-busy="true">
            <div className="w-12 h-12 border-4 border-primary/20 border-t-primary rounded-full animate-spin" aria-hidden="true"></div>
            <p className="font-bold text-medical-text-light">{t.loading}</p>
          </div>
        )}

        {error && (
          <div className="medical-card p-6 border-red-100 bg-red-50 text-red-600 font-bold mb-8" role="alert">
            {error}
          </div>
        )}
        ...
        {doctor && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            {/* Header Profile Card */}
            <article className="medical-card p-8 md:p-12 relative overflow-hidden" aria-labelledby="doctor-name">
              <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-12">
                <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
                  <div className="w-32 h-32 md:w-40 md:h-40 rounded-3xl bg-primary/10 flex items-center justify-center text-primary border-4 border-white shadow-xl" aria-hidden="true">
                  ...
                  <div className="text-center md:text-left space-y-4">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-100 text-xs font-black uppercase tracking-wider" role="status">
                      <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                      </svg>
                      Verified Specialist
                    </div>
                    <h1 id="doctor-name" className="text-4xl md:text-5xl font-black text-secondary tracking-tight">
                      {doctor?.userId?.name || "Doctor"}
                    </h1>
                    <p className="text-xl md:text-2xl font-bold text-primary tracking-tight">
                      {doctor.specialty}
                    </p>
                    <div className="flex flex-wrap justify-center md:justify-start gap-3" aria-label="Qualifications">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {/* Sidebar Info */}
              <aside className="lg:col-span-1 space-y-8" aria-labelledby="clinic-info-heading">
                {clinic && (
                  <section className="medical-card p-8 space-y-8">
                    <div>
                      <h2 id="clinic-info-heading" className="text-lg font-black text-secondary uppercase tracking-tight mb-6 flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-black text-secondary uppercase tracking-tight mb-4">Languages Spoken</h2>
                      <div className="flex flex-wrap gap-2" role="list">
                        {doctor.languages?.map((l) => (
                          <span key={l} role="listitem" className="text-sm font-bold text-medical-text-light flex items-center gap-1">
              ...
              <section className="lg:col-span-2 space-y-8" aria-labelledby="reviews-heading">
                <div className="medical-card p-8">
                  <div className="flex items-center justify-between mb-10 pb-6 border-b border-slate-50">
                    <h2 id="reviews-heading" className="text-2xl font-extrabold text-secondary tracking-tight">
                      {t.patientReviews}
                    </h2>
                    {reviews.length > 0 && (
                  ...
                  {fetchingReviews ? (
                    <div className="flex h-40 items-center justify-center" aria-busy="true">
                      <div className="w-8 h-8 border-4 border-primary/20 border-t-primary rounded-full animate-spin" aria-hidden="true"></div>
                    </div>
                  ) : reviews.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                      <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center text-slate-200" aria-hidden="true">
                    ...
                    <div className="space-y-8" role="list">
                      {reviews.map((r) => (
                        <article key={r._id} role="listitem" className="p-6 rounded-2xl bg-slate-50 border border-slate-100 space-y-4 hover:border-primary transition-all">
                          <div className="flex justify-between items-start">
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 rounded-xl bg-secondary text-white flex items-center justify-center font-black text-xl" aria-hidden="true">
                            ...
                            <time dateTime={r.createdAt} className="text-[10px] font-black text-medical-text-light uppercase tracking-widest">
                  ...
                  {token && user?.role === "patient" && (
                    <section className="mt-16 pt-12 border-t border-slate-100" aria-labelledby="write-review-heading">
                      <h3 id="write-review-heading" className="text-xl font-extrabold text-secondary tracking-tight mb-8">
                        {t.shareExperience}
                      </h3>
                      <form onSubmit={handleReviewSubmit} className="space-y-8">
                        <div className="space-y-4">
                          <label id="star-rating-label" className="medical-label">Select Star Rating</label>
                          <div className="flex gap-3" role="radiogroup" aria-labelledby="star-rating-label">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <button
                                key={star}
                                type="button"
                              ...
                              >
                                ...
                                {userRating === star && (
                                  <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-primary animate-pulse" aria-hidden="true"></div>
                                )}
                              </button>
                            ))}
                          </div>
                        ...
                        <div className="space-y-4">
                          <label htmlFor="review-comment" className="medical-label">Your Experience</label>
                          <textarea
                            id="review-comment"
                            value={userComment}
                          ...
                          <div className="flex justify-end">
                            <span className="text-[10px] font-black text-medical-text-light uppercase tracking-widest" aria-live="polite">
                        ...
                        {formError && (
                          <div className="p-4 rounded-xl bg-red-50 border border-red-100 text-red-600 font-bold text-sm flex items-center gap-3" role="alert">
